...
public class ConsultController {
    ...
    public ResponseEntity<Void> save(@Valid @RequestBody ConsultListExamDTO dto) throws Exception{
        ...
        cons.setConsultDate(LocalDateTime.now());
    ...
    public ResponseEntity<byte[]> exportToExcel(
            ...
            @RequestParam(required = false) String date2) throws IOException {

        byte[] bytes = service.exportToExcel(date1, date2);

        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_OCTET_STREAM);
        headers.set(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=consults.xlsx");

        return new ResponseEntity<>(bytes, headers, HttpStatus.OK);
    }

    @GetMapping("/exportExcel/{id}")
    public ResponseEntity<byte[]> exportToExcelByConsult(@PathVariable("id") Integer id) throws IOException {
        byte[] data = service.exportToExcelByConsult(id);

        HttpHeaders headers = new HttpHeaders();
        headers.set(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=consult_" + id + ".xlsx");
        headers.setContentType(MediaType.APPLICATION_OCTET_STREAM);
    ...
    public ResponseEntity<Void> reschedule(
            ...
            @RequestBody @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime newDate
    ) {
